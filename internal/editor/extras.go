...
// cmdIspellWord checks the spelling of the word at point using the spell checker.
...
// ---------------------------------------------------------------------------
// List navigation (forward-list / backward-list)
// ---------------------------------------------------------------------------
...
func (e *Editor) cmdForwardList() {
	...
	for i < n {
		...
		case ')', ']', '}':
			if depth == 0 {
				// We hit a closer without a matching opener — exit the enclosing list.
				buf.SetPoint(i + 1)
				return
			}
			depth--
			if depth == 0 {
				// We completed a balanced expression.
				buf.SetPoint(i + 1)
				return
			}
		default:
			if mode == "bash" && isWordRune(r) {
				// Only act at the start of a word.
...
func (e *Editor) cmdBackwardList() {
	...
	for i >= 0 {
		...
		case '(', '[', '{':
			if depth == 0 {
				// We hit an opener without a matching closer — exit the enclosing list.
				buf.SetPoint(i)
				return
			}
			depth--
